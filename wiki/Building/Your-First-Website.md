...
<head>
  ...
  <style>
    ...
    h1 { font-size: 2rem; }
    p  { color: #e8e8e8; max-width: 480px; margin: 12px auto; }
  </style>
</head>
<body>
...
- `<style>` — CSS lives here. Every rule is "target: { property: value; }" — `body { background: #0a0a0a; }` reads as "body: paint it near-black."
- `#0a0a0a` / `#00ff66` — color codes (hex). The first is the block's near-black; the second is **Hustle Green** — the official FED-EDU accent. Same colors as the whole site (styles.css).
...
2. Change `#00ff66` to `#ff0066` — refresh — note the color flip. You just redecorated.
3. Add a second `<p>` line under the first.
4. Add `border: 2px solid #00ff66; padding: 20px;` inside the `h1 { }` block — box the headline.
